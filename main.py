...
logger = logging.getLogger("tywan")
...
class Tywan:
    ...
    def _update_tray(self, status_text: str, state: str = "idle"):
        """更新系統列圖標外觀與提示文字"""
        if self.tray_icon:
            self.tray_icon.title = f"tywan - {status_text}"
            try:
    ...
    def _create_tray_icon(self):
        ...
        try:
            ...
            menu = pystray.Menu(
                pystray.MenuItem("tywan v0.1.0", None, enabled=False),
            ...
            self.tray_icon = pystray.Icon("tywan", img, "tywan - 就緒", menu)
    ...
    def _quit(self, icon=None, item=None):
        logger.info("Shutting down tywan...")
        self.hotkey.stop()
        if self.tray_icon:
            self.tray_icon.stop()
        sys.exit(0)
    ...
    def run(self):
        ...
        logger.info("tywan started! Hold %s to speak", hotkey)
        try:
            while True:
                time.sleep(1)
